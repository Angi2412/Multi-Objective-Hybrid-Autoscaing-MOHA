...
def calc_eval_metrics(c: pd.DataFrame, n: pd.DataFrame, dir_path: str):
    ...
    result["metric 50/50"] = 0.5 * response_time_exceeded + 0.25 * (cpu_out_threshold + memory_out_threshold) + 0.25 * n["kube_deployment_spec_replicas"].mean()
    result["metric 60/40"] = 0.6 * response_time_exceeded + 0.20 * (cpu_out_threshold + memory_out_threshold) + 0.20 * n["kube_deployment_spec_replicas"].mean()
